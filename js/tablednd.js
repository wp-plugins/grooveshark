...
//
// Additional Notice:
// This script was modified to work specifically with the Grooveshark Wordpresss Plugin.
// Do not use this directly with your project. Download the script from the original
// script homepage instead.
...
document.onmouseup   = function(ev){
    if (currenttable && currenttable.dragObject) {
        ...
        //Updates table rows for the Grooveshark Wordpress plugin
        setTimeout("updateTableRows()",500);
        //currenttable = null; // let go of the table too
    }
}

//Following function needed for Grooveshark Wordpress Plugin
...
function TableDnD() {
    ...
    this.addDraggableRow = function(item) {
        if (!item) return;
        var self = this;
        item.onmousedown = function(ev) {
            var target = getEventSource(ev);
            if (target.tagName == 'INPUT' || target.tageName == 'SELECT') return true;
            currenttable = self;
            self.dragObject = this;
            self.mouseOffset = self.getMouseOffset(this, ev);
            return false;
