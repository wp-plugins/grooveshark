...
function groovesharkFullSetup(gsDataStore) {
    ...
    jQuery('#widgetWidth, #widgetHeight', context).keydown(function(ev) {
        if (ev.which == 13) {
...
function addToSelectedPlaylist(obj) {
    ...
    jQuery.each(playlistSongs, function(i, song) {
        if ((!!song.songNameComplete) && (!!song.songID)) {
            count++;
            newSongContent.push("<tr class='", (count % 2 ? altClass : 'gsTr1'), " newRow'><td class='gsTableButton'><a title='Play this song' class='gsPlay' name='", song.songID, "' style='cursor: pointer;'></a></td><td>", song.songNameComplete, "<input type='hidden' name='songsInfoArray[]' class='songsInfoArrayClass' value='", song.songID, "'/></td><td class='gsTableButton'><a title='Remove This Song' class='gsRemove' style='cursor: pointer; float: right;'></a></td></tr>");
    ...
    if (songsArray.length > 0) {
        ...
        if (songIDs.length == 1) {
            ...
            if (displayOption == 'widget') {
                ...
                // save dashboard and playlist options
                if ((sidebarOption == 1) || (dashboardOption == 1)) {
                    var sidebarEmbed = getPlaylistGSWidget(songIDs, 200, widgetHeight, colorArray[1], colorArray[0], colorArray[0], colorArray[2], colorArray[1], colorArray[0], colorArray[2], colorArray[1], colorArray[1], colorArray[0], colorArray[2], colorArray[1], colorArray[1], colorArray[2], colorArray[1]);
                    
                    jQuery.post(document.getElementById('gsBlogUrl').value + '/wp-content/plugins/grooveshark/gsSaveEmbed.php', {embedCode: widgetEmbed, sidebarEmbed: sidebarEmbed, sidebarOption: sidebarOption, dashboardOption: dashboardOption});
                }
        ...
        gsStatusMessage.show().html('Your widget is in your comment.');
